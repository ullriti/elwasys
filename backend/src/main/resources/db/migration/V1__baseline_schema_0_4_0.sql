-- Flyway-Baseline: konsolidierte Zusammenfassung von Common/resources/database-init.sql
...
--     Migration.
--   - Common/resources/database-upgrade/upgrade_0.3.1_0.3.2.sql und upgrade_0.4.0.sql sind
--     NICHT separat nachgebildet: database-init.sql selbst enthält bereits deren Endzustand
--     (deconz_uuid-Spalte, die aktuellen device_program_rel-FK-Definitionen, das
--     db.version-Seed '0.4.0'), sie wurden inhaltlich bereits in database-init.sql gepflegt.
--     Ein Vergleich (siehe backend/README bzw. Testreport im Abschlussbericht dieses
...
--     Common/resources/database-upgrade/ werden nicht mehr fortgeschrieben.
--
-- Rollen/Grants sind in DO-Blöcke mit Existenzprüfung gefasst: PostgreSQL-Rollen sind
-- Cluster-weit (nicht pro Datenbank), daher würde ein CREATE USER/CREATE GROUP fehlschlagen,
-- wenn dieselbe Rolle bereits (in einer anderen Datenbank desselben Clusters) existiert - z.
-- B. weil dort bereits eine über den Alt-Weg angelegte Bestands-DB liegt. Für Client/Portal
-- ist das Ergebnis identisch: dieselben Rollennamen mit denselben Rechten.
...
/* USERS & PERMISSIONS */
-- Rollenanlage ist idempotent gefasst (siehe Erklärung oben): Rollen sind Cluster-weit,
-- nicht pro Datenbank.
DO
$do$
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_catalog.pg_roles WHERE rolname = 'elwaportal') THEN
    EXECUTE 'CREATE USER elwaportal';
  END IF;
