...
Werkzeuge für die **Produktivumschaltung** (docs/kb/05-migration-plan.md, "Phase 6 –
...
Produktiv-DB ist NICHT Teil dieses Arbeitspakets (siehe docs/kb/05-migration-plan.md,
...
- Backend-Jar gebaut (`mvn -N install -DskipTests` dann
  `mvn -f pom.xml package -pl backend -DskipTests`, siehe docs/kb/04-build-and-run.md) bzw. das
  ...
  docs/kb/04-build-and-run.md, "Wichtiger Fund Phase 4 AP4" - ohne Produktionsmodus bricht ein
...
docs/kb/04-build-and-run.md "Lokal starten" bzw. "Deployment (Produktion)" für Docker-
Compose/Helm):
```bash
ELWASYS_DB_URL=jdbc:postgresql://<host>:5432/elwasys \
ELWASYS_DB_USER=elwaportal ELWASYS_DB_PASSWORD=<Passwort> \
...
(dünner Wrapper um das bestehende `token-cli`-Profil, siehe docs/kb/04-build-and-run.md). Das
