...
> **Zielarchitektur (Stand Phase 5, siehe docs/kb/05-migration-plan.md)**: Root-Reactor mit 2
...
| ~~Portal~~ | ~~`webportal`~~ | – | – | – | **Entfernt (Phase 5 AP1)** – ehemalige eigenständige Vaadin-7-Admin-Weboberfläche, fachlich abgelöst durch das ins Backend eingebettete Portal-UI seit Phase 3 AP6 (volle Feature-Parität, siehe docs/kb/06-ui-tests.md) |
...
Parent-POM, siehe docs/kb/04-build-and-run.md).

## Namespaces (Java-Packages)

- `org.kabieror.elwasys.common` – kleine gemeinsame Bibliothek (seit Phase 5 AP1 auf 6 Klassen
  geschrumpft, siehe docs/kb/03-modules.md); das eigenständige Common-Modul ist im Phase-5-Nachtrag
...
`org.kabieror.elwasys.common`, siehe docs/kb/03-modules.md) liegen unverändert im Client-Raspi-Modul
...
- **JavaFX 23.0.2** (`javafx-controls`, `javafx-fxml`, `javafx-web`) – UI über FXML
  *(Phase 4 AP2, 2026-07-21: 20 → 23.0.2, höchste stabile Version, die noch auf dem
  festgelegten Java-21-Client läuft – siehe docs/kb/05-migration-plan.md)*
...
- deCONZ-REST/Event-API: `java.net.http` (JDK, kein Fremd-Client) – unirest/HttpComponents
  wurden hierfür entgegen einer älteren Annahme nie gebraucht, siehe docs/kb/03-modules.md
- Gson `2.10.1` – JSON für die REST-API v1 (`api/ApiClient`, seit Phase 4 AP4 der primäre
  Datenzugriffspfad, siehe docs/kb/03-modules.md)
- ~~Pushover-Client, Commons Email~~ – entfernt (Phase 4 AP4): Benachrichtigungsversand
  läuft jetzt zentral über das Backend, siehe docs/kb/05-migration-plan.md
...
Ehemals Vaadin 7.6.8 + GWT 2.7.0, WAR/Jetty, siehe docs/kb/05-migration-plan.md (Änderungslog
„Phase 5 AP1") für den letzten Stand vor der Entfernung. Fachlich vollständig abgelöst durch
das Vaadin-Flow-Portal-UI im Backend (siehe unten und docs/kb/03-modules.md, „Portal-UI (Vaadin
Flow)").
...
- Test: JUnit 5, Testcontainers (Default) mit lokalem PostgreSQL-Override für Docker-lose
  Umgebungen (siehe docs/kb/04-build-and-run.md); **keine** Abhängigkeit auf `common` mehr – die 3
...
- Build: `spring-boot-maven-plugin` (repackage) → lauffähiges Jar; Modul-Property
  ...
  build (siehe docs/kb/04-build-and-run.md)
...
- **`ElwaManager`** (Singleton) – zentraler Manager, hält Verweise auf Configuration,
  ...
  Datenzugriffspfad, siehe docs/kb/03-modules.md), `ws/TerminalWebSocketClient` (seit Phase 4
...
- **`ws/TerminalWebSocketClient`** *(neu, Phase 4 AP5)* – die ausgehende WebSocket-Verbindung
  ...
  ist umgedreht, siehe „Kommunikationswege“ unten und docs/kb/03-modules.md.
...
Komponentenliste: docs/kb/03-modules.md, Abschnitt „Portal-UI (Vaadin Flow)".
...
**Endzustand seit Phase 5** (siehe docs/kb/05-migration-plan.md, Zielarchitektur): der
...
1. **Terminal ⇄ Backend REST** (`/api/v1/**`, Standort-Token im `Authorization: Bearer`-
   ...
   Client-Raspi (`api/ApiClient` in `Client-Raspi/.../api/`, siehe docs/kb/03-modules.md).
...
sind in docs/kb/05-migration-plan.md (Änderungslog, Phasen 2–5) im Detail nachvollziehbar.

Vollständige Endpunkt-/Nachrichtenreferenz: docs/kb/03-modules.md (Abschnitt Backend, AP4/AP5).
