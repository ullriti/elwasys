...
> eigenes E2E vorhanden, siehe docs/kb/03-modules.md).
...
Neues Modul `backend/` (Spring Boot 3.x, siehe docs/kb/01-architecture.md, docs/kb/03-modules.md).
...
wird in den Tests lokal über den Helfer `LegacySha1` reproduziert (siehe docs/kb/03-modules.md):
...
- **Ohne Docker (diese Remote-Sandbox – verifiziert: `docker ps` scheitert mit „no such file
  ...
  docs/kb/05-migration-plan.md Änderungslog AP2).
...
V1-Baseline die einzige Schema-Quelle ist. Details/Ergebnis siehe docs/kb/02-data-model.md und
docs/kb/05-migration-plan.md (Änderungslog, Phase 2 AP1).
...
bedienen kann (Details/Klärungsbedarf siehe docs/kb/05-migration-plan.md, Risikotabelle + „Offene
Fragen"). Betrifft NICHT `backend/run-backend-tests.sh` (erzwingt
`vaadin.productionMode=true`, siehe docs/kb/03-modules.md, Abschnitt „Portal-UI (Vaadin Flow)“).
...
abbrach – siehe docs/kb/05-migration-plan.md, Änderungslog „Phase 4 AP4" für die vollständige
Ursachenanalyse. Fix: `start-test-backend.sh` baut jetzt ebenfalls mit `-Pproduction`
(dieselbe Umgehung wie `backend/e2e/scripts/start-backend.sh`, siehe docs/kb/06-ui-tests.md).
...
Passwort zu geben (siehe `V7__remove_default_admin_password.sql`, docs/kb/02-data-model.md,
docs/kb/05-migration-plan.md).
...
Hintergrund/Roadmap in docs/kb/05-migration-plan.md ("Phase 6 – Produktivumschaltung"):
...
docs/kb/03-modules.md). **`database.*` und `maintenance.server`/`maintenance.port`/
...
zentral über das Backend, siehe docs/kb/03-modules.md „Benachrichtigungsdienst"). **Seit Phase 4
AP6**: `offline.pollIntervalSeconds` (Default 20) steuert das Intervall des periodischen
Offline-Abgleichs (Snapshot-Aktualisierung + Journal-Replay, siehe docs/kb/03-modules.md
...
(`TerminalReadinessMarker`, siehe docs/kb/03); bleibt der `mtime`-Fortschritt binnen
...
- **Client**: One-Line-Setup auf frischem Raspberry Pi:
  ...
  docs/kb/05-migration-plan.md).
- **Backend** (seit Phase 2 AP6, siehe docs/kb/05-migration-plan.md): Container-Image
...
docs/kb/05-migration-plan.md AP6).
...
in Phase 5 AP1, 2026-07-21 entfernt – siehe docs/kb/05-migration-plan.md, docs/kb/06-ui-tests.md)*:
...
- 3 parallele Jobs: **client** (inkl. Cross-Component-Suite P21/P22) /
  **backend-e2e** / **backend** – Build + Tests, spiegeln die
  lokalen Runner-Skripte (`run-ui-tests.sh` etc., siehe docs/kb/06) bzw. für Backend
...
- ~~Vaadin 7 / GWT 2.7 Widgetset-Compilation: langsam, speicherhungrig, alte Repos
  ...
  nutzt npm-freies, modernes Tooling ohne GWT-Widgetset-Compilation (siehe docs/kb/03-modules.md).
- ~~Alte Plugin-/Dependency-Versionen (Postgres 9.3-Treiber im Portal).~~ Gegenstandslos seit
  ...
  `Client-Raspi/pom.xml` entfernt (siehe docs/kb/05-migration-plan.md) – Client-Raspi hat damit
...
- **Raspi-Terminal-Laufzeit vs. Build-Sprachlevel**: `setup.sh` installiert seit
  ...
  docs/kb/05-migration-plan.md (Zeile als aufgelöst markiert).
